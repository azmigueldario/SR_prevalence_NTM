...
- **NTM infection** point/annual prevalence = 67 (including those only reporting %)
...
clean_data %>% 
  ...
  # 57 report point_infection and remaining 10 annual period_infection
...
* For analysis, we create a dataset that includes only the set of studies to analyze
    ...
    - Uses only last registry of every region to avoid over-representation too
...
# n = 21
# n =  4 were cohort studies using registry data
# n = 34 registry studies over-represented
```
...
```{r}
# Subgroup test is not significant but close (0.07)
...
Once we removed over-represented registry studies, no significant differences. 
...
# baseline
transf.ilogit(-3.1483)
# sample size < 1000
transf.ilogit(-3.1483 + 1.7381)
# other regions
transf.ilogit(-3.1483 - 1.1369)
...
# Exclude studies that used registry data 


```{r}
# overall result
inf_point_data %>% 
  filter(study_design!="Registry report" & used_registry !="no") %>% 
  metaprop_formatted() %>% 
forest_wrapper()
