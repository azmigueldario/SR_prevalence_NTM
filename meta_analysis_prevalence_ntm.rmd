...
temp <-  list()
temp$input <- 
  read_csv("./input/20211108-data_meta_analysis_qa.csv")  %>% 
...
rob_data <- 
  clean_data %>% 
  select(study, matches("Q[0-9]"), Overall_risk, study_design, before_year) %>% 
  as.data.frame() 
```
...
Thus, we summarize the quality of studies after sub-setting the data properly. 
We first produce different summary tables and then a few traffic light plots

```{r, eval=F}
rob_data$study_design
rob_data %>% 
  rob.summary(data=., 
            name.high="High", 
            name.unclear="Unclear",
...
clean_data %>%
  filter(study_design=="Registry report",
         grepl("Usa|Australia|Brazil", study)) %>%
  select(study, matches("Q[0-9]"), Overall_risk) %>%
  rename(Overall=Overall_risk) %>% 
  as.data.frame() %>%
  robvis::rob_traffic_light(
    data=.,
    tool = "Generic",
...
# region
filter(clean_data, period_prevalence=="yes") %>% 
  count_table(region1)
# population
filter(clean_data, period_prevalence=="yes") %>% 
  count_table(age_group)
...
filter(clean_data, period_prevalence=="yes") %>% 
  mutate(
    study_length=last_year_data-first_year_data+1) %>% count(study_length) %>% 
...
# view results according to study and time-interval
  ...
  mutate(
    ...
    select(study, study_length, pp,
           all_ntm_infection, sample_size_cf) %>%  view
