...
- Create a new variable that specifies studies that used registry data 
- Create an indicator variable for registry_reports
...
## Exploratory: study design, region, age
...
## Exploratory: years of data collection 

```{r}
# relevel before_year factor
clean_data$before_year<- factor(clean_data$before_year, 
                                levels = c("2000 or before", "2001-2004", "2005-2009", 
...
## Exploratory: infection period prevalence
...
temp$period_infection <- 
  ...
  mutate(estimate = case_when( is.na(period_inf_perc) ~ (inf_events_period / sample_size_period ) * 100,
...
# study_period length
temp$period_infection %>% 
  count(study_length = last_year_data - first_year_data + 1) %>%
...
temp$period_infection %>%
  mutate(study_length = last_year_data - first_year_data + 1) %>% 
    select(study, study_length, estimate, 
...
metareg_data <- 
  ...
  mutate(region1 = relevel(region1, ref = "NAM"),
         ...
         sample_size_cat = case_when(sample_size_cf < 300 ~ "small", 
                                     TRUE ~ "large"),
         sample_size_cat = factor(sample_size_cat, levels = c("small", "large")),
...
2. We will use Logit transformation (`PLO`) to optimize the properties of the generalized linear model as recommended by __(Schwarzer 2019)__
    ...
    * Model with all pre-specified predictors failed to converge, the best prediction was obtained using `sample_size_cat`. Other covariates are included as pre-specified.
...
temp$coef.names <- c("Intercept", "Design: cohort study", 
...
transf.ilogit(-1.589)
# large sample size
transf.ilogit(-1.589 - 0.9305)
# europe
transf.ilogit(-1.589 - 0.9631)
# other regions
transf.ilogit(-1.589 - 2.0525)

```

## Unspecified: multilevel model
...
## Unspecified: subgroup by overall quality
