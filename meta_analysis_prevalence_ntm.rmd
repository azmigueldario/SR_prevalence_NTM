...
temp$untidy_data<- 
  ...
  mutate(study_design=case_when(study_design=="Cross-sectional study" ~ "Cross-sectional study",
         ...
         is_registry=case_when(study_design=="Registry report" ~ 1,
                               TRUE ~ 0),
         size_cat_inf = case_when(sample_size_cf <= 300 ~ "small",
                                  sample_size_cf <= 1000 & sample_size_cf > 300 ~ "medium",
...
metareg_data <- 
  ...
  mutate(region1 = relevel(region1, ref = "NA"),
         ...
         sample_size_cat = case_when(sample_size_cf <= 1000 ~ "small", 
                                     TRUE ~ "large"),
...
metareg_glmm_wrapper(covs=~ design_or_registry ) %>% 
  AIC()
metareg_glmm_wrapper(covs=~ design_or_registry + sample_size_cat) %>% 
  AIC()
metareg_glmm_wrapper(covs=~ design_or_registry + sample_size_cat + region1) %>% 
  AIC()
metareg_glmm_wrapper(covs=~ design_or_registry + sample_size_cat + region1 + before_year) 
...
temp$coef.names <- c("Intercept", 
                      "Design: cohort study", "Design: registry data", 
...
transf.ilogit(-0.75)
# large sample size
transf.ilogit(-0.75 - 0.8715)
# europe
transf.ilogit(-0.75 - 0.8703)
# other regions
transf.ilogit(-0.75 - 2.0722)
# before 2000
transf.ilogit(-0.75 - 1.26)
