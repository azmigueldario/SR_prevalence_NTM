...
ii.	Subgroup by high quality studies according to quality appraisal _done_
...
iii. Corroborate that the numbers in the UK and ECFS registries match _#done, umbers match properly_ 
...
```{r, message=FALSE}
getwd()
prevalence <- read_csv("D:/Git/SR_prevalence_NTM/20210809-data_meta_analysis_qa.csv")  %>% 
...
As the variable region is going to be used as a grouping variable in the meta-analysis and some of the levels, Australia(AUS) and Middle-East (ME), have small numbers in the subgroup. We record them together with LatinAmerica(LAC) into a category called Other

```{r}
table(prevalence1$region) %>% 
  kable()
...
prevalence1 <- 
  ...
  mutate(Overall_Q=case_when(
    ...
    Q1_Sampling_frame=="Low" & Q2_Sampling_scheme=="Low" & Q4_Population_description=="Low" & 
      Q7_Standardized_measurement!="High" & Q8_Statistical_calculation=="Low" ~ "Low",
    TRUE ~ "Unclear")) 
...
Here, we plot the subgroup analysis according to study groups
